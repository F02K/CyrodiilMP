...
$enabledPath = Join-Path $modsPath 'enabled.txt'
...
if ($enabled -notcontains 'CyrodiilMP_AutoUSMAP') {
    Add-Content -LiteralPath $enabledPath -Value 'CyrodiilMP_AutoUSMAP'
}

Write-Host 'Installed CyrodiilMP_AutoUSMAP.'
Write-Host "Mod path: $modPath"
Write-Host "Enabled list: $enabledPath"
Write-Host ''
