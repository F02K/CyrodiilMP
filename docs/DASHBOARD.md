...
- Detect whether the UE4SS research helpers are installed into the local game folder, then run the helper installer from the browser when needed.
...
- The UE4SS install helper will create the missing `Win64\Mods` folder automatically when the saved game path looks like a valid Oblivion Remastered install root. It installs research/dumper helpers only, not the multiplayer runtime.
